      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
        <form className="glass space-y-4 p-6" onSubmit={handleSubmit}>
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-brandPurple/15 text-brandPurple">
            ...
            <div>
              <p className="text-sm font-semibold text-navy-900">Nova meta</p>
              <p className="text-xs text-slateSoft-500">Planeje viagens, casa própria ou reserva de emergência.</p>
            </div>
          </div>
          <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input className="input mt-2" type="number" min="0" step="0.01" value={targetAmount} onChange={(e) => setTargetAmount(e.target.value)} required />
            </div>
            <div>
              <label className="label">Valor atual</label>
              <input className="input mt-2" type="number" min="0" step="0.01" value={currentAmount} onChange={(e) => setCurrentAmount(e.target.value)} required />
            </div>
          </div>
          <button className="btn-primary w-full" type="submit">Criar meta</button>
        </form>

        <div className="grid gap-4 sm:grid-cols-2">
          {goals.length ? goals.map((goal) => <GoalCard key={goal.id} goal={goal} />) : (
            <div className="glass p-5">
              <p className="text-sm text-slateSoft-500">Adicione metas como Viagem, Reserva de emergência ou Comprar uma casa.</p>
            </div>
          )}
        </div>
      </div>
